...
@router.post("/usuarios/registrar", response_model=UsuarioResponse, status_code=status.HTTP_201_CREATED)
...
def login_usuario(user: UsuarioLogin):
    ...
    if not usuario_db or not verify_password(user.senha, usuario_db["senha_hash"]):
        raise HTTPException(
