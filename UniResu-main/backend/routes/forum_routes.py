...
def listar_topicos():
    """Lista todos os tópicos do fórum. Esta rota é pública."""
    db = get_db()
    if db is None:
...
def criar_topico(
    ...
    """Cria um novo tópico no fórum. Requer que o utilizador esteja autenticado."""
    db = get_db()
...
def adicionar_resposta(
    ...
    """Adiciona uma resposta (dúvida) a um tópico existente. Requer autenticação."""
    db = get_db()
